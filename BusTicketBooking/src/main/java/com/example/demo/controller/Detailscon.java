...
//GETSTUDENT BY ID AND NAME USING SQL
	...
	@GetMapping("showquerys/{name}")
	public List<Details> showdetail(@PathVariable String name){
		return se.getDetail(name);
	}
...
//UPDATEID BY USING SQLS
	@PutMapping("updatequerys/{name}/{oldid}")
	public String updatestudent(@PathVariable String name,@PathVariable int oldid) {
		return se.updateDetails(name, oldid)+"UPDATED";
	}
	
	@PutMapping("updatequery/{name}/{emailid}")
	public String updatestudentname(@PathVariable String name,@PathVariable String emailid) {
		return se.updatename(name, emailid)+"UPDATED";
	}
